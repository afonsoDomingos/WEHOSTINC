...
export default function DashboardNav({ userName, onLogout, onRefresh }: DashboardNavProps) {
  try {
    ...
    console.error('Erro no DashboardNav:', err);
    return (
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-3.5">
            <span className="text-sm font-bold text-gray-900">WEHOSTHERE</span>
            <button
              type="button"
              onClick={onLogout}
              className="px-3 py-2 text-sm font-medium text-gray-700 hover:text-red-600 rounded-lg transition"
            >
              Sair
            </button>
          </div>
