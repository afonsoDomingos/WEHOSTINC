...
import { ArrowLeft, BookOpen, ChevronLeft, ChevronRight, CheckCircle, Menu, X, Video, FileText } from 'lucide-react';
...
export default function CourseViewPage() {
  ...
  const getCurrentLesson = () => {
    ...
    const module = modules[currentChapter];
  ...
  const handleCompleteLesson = () => {
    ...
    setToast({ show: true, message: 'Capítulo concluído!', type: 'success' });
    ...
    if (currentLesson < moduleLessons.length - 1) {
      ...
      if (currentChapter < modules.length - 1) {
  ...
  const goToPrevious = () => {
    if (currentLesson > 0) {
      ...
      const prevModule = modules[currentChapter - 1];
  ...
  const goToNext = () => {
    ...
    if (currentLesson < moduleLessons.length - 1) {
      setCurrentLesson(currentLesson + 1);
    } else if (currentChapter < modules.length - 1) {
      setCurrentChapter(currentChapter + 1);
      setCurrentLesson(0);
    }
  };
  ...
  const currentLessonData = getCurrentLesson();
  const currentModule = modules[currentChapter];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        ...
        <div className={`fixed inset-y-0 left-0 z-40 w-80 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out lg:relative lg:transform-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
          <div className="h-full flex flex-col">
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              <h2 className="font-semibold text-gray-900">Índice</h2>
              <button
              ...
              {modules.map((module, index) => {
                ...
                return (
                  <div key={module.id} className="mb-4">
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <BookOpen className={`h-4 w-4 ${isActive ? 'text-primary-600' : 'text-gray-400'}`} />
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 mt-1">{completedInModule}/{moduleLessons.length} concluídos</div>
                        </div>
                      </div>
                    </button>
                  </div>
        ...
        {/* Main Content - Webbook style */}
        <div className="flex-1 min-h-screen">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            {/* Chapter Header */}
            <div className="mb-8">
              <div className="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                <span>Capítulo {currentChapter + 1}</span>
                <span>/</span>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <div className="p-6 sm:p-8">
                  ...
                  <div className="mb-6 pb-6 border-b border-gray-200">
                    <div className="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                      <span>Lição {currentLesson + 1}</span>
                      <span>/</span>
                      <span>{getModuleLessons(currentModule.id).length}</span>
                    </div>
                    <h3 className="text-2xl font-bold text-gray-900 mb-3">{currentLessonData.title}</h3>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                    <button
                      ...
                      className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition ${
                        currentChapter === 0 && currentLesson === 0
                    ...
                    <button
                      onClick={goToNext}
                      disabled={currentChapter === modules.length - 1 && currentLesson === getModuleLessons(currentModule.id).length - 1}
                      className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition ${
                        currentChapter === modules.length - 1 && currentLesson === getModuleLessons(currentModule.id).length - 1
                          ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                          : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                      }`}
                    >
