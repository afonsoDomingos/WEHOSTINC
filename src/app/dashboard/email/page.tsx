    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Contas de Email Corporativo</h1>
                  <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5">Gerencie os emails da sua empresa, altere senhas e aceda ao Webmail</p>
                </div>
                <button
                  onClick={() => setShowCreateModal(true)}
                  className="flex items-center justify-center space-x-2 bg-primary-600 text-white font-bold text-xs sm:text-sm px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl hover:bg-primary-700 transition cursor-pointer shadow-sm w-full sm:w-auto"
                >
              ...
              {emails.length === 0 ? (
                <div className="text-center py-8 sm:py-12">
                  ...
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">Nenhuma conta configurada</h3>
                  <p className="text-gray-600 mb-3 sm:mb-4 text-xs sm:text-sm">Crie sua primeira conta de email profissional personalizada</p>
                  <button
                    onClick={() => setShowCreateModal(true)}
                    className="flex items-center justify-center space-x-2 bg-primary-600 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl hover:bg-primary-700 transition mx-auto font-bold text-xs sm:text-sm w-full sm:w-auto"
                  >
                ...
                <div className="space-y-3 sm:space-y-4">
                  {emails.map((email) => (
                    <div key={email.id} className="border rounded-2xl p-3 sm:p-4 sm:p-5 hover:bg-gray-50/80 transition bg-white shadow-sm">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3">
                        <div className="flex items-center space-x-2.5 sm:space-x-3 sm:space-x-4 w-full sm:w-auto">
                          <div className="bg-primary-50 p-2 sm:p-2.5 sm:p-3 rounded-2xl border border-primary-100 shrink-0">
                            <Mail className="h-4 w-4 sm:h-5 sm:w-5 sm:h-6 sm:w-6 text-primary-600" />
                          </div>
                          <div className="min-w-0 flex-1">
                            <h3 className="font-bold text-gray-900 text-xs sm:text-sm sm:text-base flex items-center space-x-2">
                              <span className="truncate">{email.email}</span>
                            </h3>
                            <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5">Conta Corporativa Profissional</p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 w-full sm:w-auto justify-end">
                          ...
                          <Link
                            ...
                            className="px-2.5 py-1.5 sm:px-3 sm:px-3.5 sm:py-2 bg-primary-600 hover:bg-primary-700 text-white font-bold text-[10px] sm:text-xs rounded-xl transition flex items-center space-x-1 sm:space-x-1.5 cursor-pointer shadow-sm shrink-0"
                          >
                            <Mail className="h-2.5 w-2.5 sm:h-3 sm:w-3 sm:h-3.5 sm:w-3.5" />
                      ...
                      {email.status === 'pending' && (
                        <div className="mt-2 sm:mt-3 p-2 sm:p-3 bg-amber-50 border border-amber-200 rounded-xl text-[10px] sm:text-xs text-amber-900 flex items-center space-x-2">
                      ...
                      <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 text-[9px] sm:text-[10px] sm:text-xs text-gray-600">
                        <div>
                        ...
                        <div className="col-span-2 sm:col-span-1">
                          <button
                            type="button"
                            onClick={() => handleOpenEditModal(email)}
                            className="text-primary-600 hover:text-primary-800 font-bold underline cursor-pointer flex items-center space-x-1 text-[9px] sm:text-[10px] sm:text-xs"
                          >
                            <Key className="h-2 w-2 sm:h-2.5 sm:w-2.5 sm:h-3 sm:w-3" />
            ...
            <div className="bg-white border border-blue-200 rounded-2xl p-3 sm:p-4 sm:p-6 shadow-sm">
              <div className="flex items-center space-x-2 mb-2 sm:mb-3">
                <ShieldCheck className="h-4 w-4 sm:h-5 sm:w-5 text-primary-600" />
                <h3 className="font-bold text-gray-900 text-xs sm:text-sm sm:text-base">Parâmetros de Configuração de Email (Outlook, iPhone, Android)</h3>
              </div>
              <p className="text-gray-600 text-[10px] sm:text-xs sm:text-sm mb-3 sm:mb-4">
                Utilize as configurações abaixo para adicionar estas contas ao Outlook, Thunderbird, Apple Mail ou no smartphone:
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 text-[10px] sm:text-xs">
                <div className="p-2 sm:p-3 bg-blue-50/60 rounded-xl border border-blue-100">
                  <span className="font-bold text-blue-900 block mb-1 text-[10px] sm:text-[11px] sm:text-xs">Servidor IMAP (Recomendado):</span>
                  <span className="font-mono text-gray-800 block text-[9px] sm:text-[10px] sm:text-xs break-all">mail.wehosthere.com</span>
                  <span className="text-gray-500 font-mono text-[9px] sm:text-[10px] sm:text-[11px] block">Porta 993 (SSL)</span>
                </div>
                <div className="p-2 sm:p-3 bg-blue-50/60 rounded-xl border border-blue-100">
                  <span className="font-bold text-blue-900 block mb-1 text-[10px] sm:text-[11px] sm:text-xs">Servidor POP3:</span>
                  <span className="font-mono text-gray-800 block text-[9px] sm:text-[10px] sm:text-xs break-all">mail.wehosthere.com</span>
                  <span className="text-gray-500 font-mono text-[9px] sm:text-[10px] sm:text-[11px] block">Porta 995 (SSL)</span>
                </div>
                <div className="p-2 sm:p-3 bg-blue-50/60 rounded-xl border border-blue-100">
                  <span className="font-bold text-blue-900 block mb-1 text-[10px] sm:text-[11px] sm:text-xs">Servidor SMTP (Envio):</span>
                  <span className="font-mono text-gray-800 block text-[9px] sm:text-[10px] sm:text-xs break-all">mail.wehosthere.com</span>
                  <span className="text-gray-500 font-mono text-[9px] sm:text-[10px] sm:text-[11px] block">Porta 465 (SSL) / 587 (TLS)</span>
                </div>
                <div className="p-2 sm:p-3 bg-blue-50/60 rounded-xl border border-blue-100">
                  <span className="font-bold text-blue-900 block mb-1 text-[10px] sm:text-[11px] sm:text-xs">URL Direto do Webmail:</span>
                  ...
                  <span className="text-gray-500 font-mono text-[9px] sm:text-[10px] sm:text-[11px] block">Login: Email + Senha</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {editEmailAccount && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl p-4 sm:p-5 sm:p-6 max-w-lg w-full border border-gray-100 animate-in slide-in-from-bottom sm:zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto sm:mx-4">
            <div className="flex items-center justify-between pb-3 sm:pb-4 border-b border-gray-100 mb-3 sm:mb-4">
              <div className="flex items-center space-x-2 sm:space-x-3">
                <div className="p-2 sm:p-2.5 bg-primary-50 rounded-xl border border-primary-100 text-primary-600">
                  <Key className="h-4 w-4 sm:h-5 sm:w-5" />
                </div>
                <div>
                  <h2 className="text-base sm:text-lg font-extrabold text-gray-900">Editar Conta de Email</h2>
                  <p className="text-[10px] sm:text-xs text-primary-700 font-mono font-semibold truncate">{editEmailAccount.email}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {editSuccessMsg ? (
              <div className="py-6 sm:py-8 text-center space-y-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto">
                  <Check className="h-5 w-5 sm:h-6 sm:w-6" />
                </div>
                <p className="text-sm sm:text-base font-bold text-gray-900">{editSuccessMsg}</p>
              </div>
            ) : (
              <form onSubmit={handleSaveEditEmail} className="space-y-3 sm:space-y-4">
                {editErrorMsg && (
                  <div className="p-2.5 sm:p-3 bg-red-50 border border-red-200 text-red-700 text-[10px] sm:text-xs font-semibold rounded-xl flex items-center space-x-2">
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 border border-gray-200 rounded-2xl text-xs sm:text-sm text-gray-900 outline-none focus:ring-2 focus:ring-primary-500 font-mono"
                ...
                <div>
                  ...
                  <div className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-blue-50 border border-blue-200 rounded-2xl text-xs sm:text-sm text-blue-800 font-semibold flex items-center gap-2">
                    <span className="text-xs sm:text-sm">&#9432;</span>
                    <div>
                      <div className="text-xs sm:text-sm">Armazenamento partilhado</div>
                      <div className="text-[9px] sm:text-[10px] font-normal text-blue-600">Pool total da conta Migadu (sem limite por caixa)</div>
                    </div>
                  </div>
                </div>

                <div className="p-2.5 sm:p-3 bg-gray-50 rounded-2xl border border-gray-200 space-y-1.5 text-[10px] sm:text-xs text-gray-700">
                  ...
                  <div className="flex items-center justify-between font-mono bg-white p-2 rounded-xl border border-gray-200">
                    <span className="truncate text-[10px] sm:text-[11px] text-primary-700">https://wehosthere.com/webmail</span>
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                      <a
                        href="/webmail"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[10px] sm:text-xs font-bold text-blue-600 hover:text-blue-800 underline"
                      >
                        Abrir ↗
                      </a>
                      <button
                        ...
                        className="text-[10px] sm:text-xs font-bold text-primary-600 hover:text-primary-800 ml-1 cursor-pointer"
                      >
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-2.5 pt-2">
                  <button
                    type="button"
                    onClick={() => setEditEmailAccount(null)}
                    className="sm:flex-1 py-2.5 sm:py-3 border border-gray-200 text-gray-700 font-bold text-xs sm:text-sm rounded-2xl hover:bg-gray-50 transition cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="sm:flex-1 py-2.5 sm:py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold text-xs sm:text-sm rounded-2xl transition cursor-pointer shadow-md"
                  >
                    Salvar Alterações
                  </button>
                </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl p-5 sm:p-6 max-w-md w-full border border-gray-100 animate-in slide-in-from-bottom sm:zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto sm:mx-4">
            <div className="flex items-center justify-between pb-3 sm:pb-4 border-b border-gray-100 mb-4">
              <h2 className="text-base sm:text-lg font-extrabold text-gray-900">Nova Conta de Email Corporativo</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleAddEmail} className="space-y-4">
              ...
              {userDomains.length === 0 ? (
                <div className="p-3 sm:p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start space-x-2 sm:space-x-3">
                  <AlertCircle className="h-4 w-4 sm:h-5 sm:w-5 text-amber-500 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs sm:text-sm font-bold text-amber-800">Nenhum domínio registado</p>
                    <p className="text-[10px] sm:text-xs text-amber-700 mt-0.5">Adicione um site/domínio primeiro em <Link href="/dashboard/sites" className="underline font-bold">Meus Sites</Link> para poder criar contas de email.</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 border border-gray-200 rounded-2xl text-xs sm:text-sm text-gray-900 outline-none focus:ring-2 focus:ring-primary-500 font-semibold"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 border border-gray-200 rounded-2xl text-xs sm:text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-primary-500 cursor-pointer"
                    >
                  ...
                  {newEmailPrefix.trim() && (
                    <div className="p-2.5 sm:p-3 bg-primary-50 border border-primary-200/80 rounded-2xl text-[10px] sm:text-xs text-primary-950 flex items-center justify-between">
                      <span className="font-semibold text-primary-900">Endereço final:</span>
                      <strong className="font-mono text-[10px] sm:text-xs sm:text-sm text-primary-700 font-black truncate ml-2">
                  ...
                  {sites.find(s => s.domain === selectedDomain)?.status === 'pending' && (
                    <div className="mt-2 p-2 sm:p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-[10px] sm:text-xs text-amber-900 flex items-start space-x-2">
                      <Clock className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-600 shrink-0 mt-0.5 animate-pulse" />
                      <span>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-700">
                    Senha de Acesso
                  </label>
                  <button
                    type="button"
                    onClick={() => setNewPassword(generateRandomPassword())}
                    className="text-[10px] sm:text-[11px] text-primary-600 font-bold hover:underline flex items-center space-x-1 cursor-pointer"
                  >
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-2.5 pt-2">
                ...
                <button
                  ...
                  className="sm:flex-1 py-2.5 sm:py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold text-xs sm:text-sm rounded-2xl transition cursor-pointer shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
