...
export const dataManager = {
  ...
  getSystemsForRent: (): SystemForRent[] => {
    const DEFAULT_SYSTEMS_DATA: SystemForRent[] = [
      {
        id: 'SYS-1001',
    ...
    if (typeof window === 'undefined') return DEFAULT_SYSTEMS_DATA;
    const data = localStorage.getItem('wehosthere_systems');
    if (!data) {
      localStorage.setItem('wehosthere_systems', JSON.stringify(DEFAULT_SYSTEMS_DATA));
      return DEFAULT_SYSTEMS_DATA;
    }
    try {
      const parsed = JSON.parse(data);
      return parsed.length > 0 ? parsed : DEFAULT_SYSTEMS_DATA;
    } catch {
      return DEFAULT_SYSTEMS_DATA;
    }
  },
