...
class SoundEffectManager {
  ...
  // 🖱️ Som sutil de clique / toque em botão / alternância de aba
  playClickSound(): void {
    const ctx = this.getContext();
    if (!ctx) return;
    try {
      ...
      console.warn('[SoundEffects] Error playing click sound:', e);
    }
  }

  // 📋 Som de cópia para a área de transferência (Double pop rápido)
  playCopySound(): void {
    const ctx = this.getContext();
    if (!ctx) return;
    try {
      ...
      [0, 0.06].forEach((offset, i) => {
        ...
        const gain = ctx.createGain();
        osc.type = 'sine';
        osc.frequency.setValueAtTime(i === 0 ? 1046.50 : 1318.51, now + offset); // C6 -> E6
        gain.gain.setValueAtTime(0, now + offset);
      ...
      console.warn('[SoundEffects] Error playing copy sound:', e);
    }
  }

  // ✨ Som de sucesso / ação guardada com sucesso
  playSuccessSound(): void {
    ...
    try {
      ...
      console.warn('[SoundEffects] Error playing success sound:', e);
    }
  }
  ...
  playErrorSound(): void {
    ...
    try {
      ...
      console.warn('[SoundEffects] Error playing error sound:', e);
    }
  }

  // 🎓 Som de inscrição de curso / Início de aula na Academia
  playEnrollCourseSound(): void {
    const ctx = this.getContext();
    if (!ctx) return;
    try {
      const now = ctx.currentTime;
      ...
      console.warn('[SoundEffects] Error playing enroll course sound:', e);
    }
  }

  // 🏆 Som de certificado gerado / conquistado
  playCertificateSound(): void {
    const ctx = this.getContext();
    if (!ctx) return;
    try {
      const now = ctx.currentTime;
      ...
      fanfare.forEach((freq, i) => {
        ...
        gain.gain.exponentialRampToValueAtTime(0.001, now + i * 0.1 + (i === 3 ? 0.8 : 0.4));
        osc.connect(gain);
        gain.connect(ctx.destination);
        osc.start(now + i * 0.1);
        osc.stop(now + i * 0.1 + (i === 3 ? 0.8 : 0.4));
      });
    } catch (e) {
      console.warn('[SoundEffects] Error playing certificate sound:', e);
    }
  }
}
